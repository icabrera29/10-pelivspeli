...
function CompetenciasController () {
	...
	this.cargarCompetencias = function (data){
		...
		// Oculto la plantilla
		console.log(data.competencias.length);
		...
		for (i = 0; i < cantidad; i++) {
			...
			$(divCompetencia).find('.titulo').text(data.competencias[i].nombre_competencia);
	...
	this.cargarCompetencia = function (id, data){
		...
		console.log(data);
		$(".nombre").text(data.nombre_competencia);
		$(".nombre").val(data.nombre_competencia);
